...
def init_auth_routes(app, mail, serializer):
    ...
    def signup():
        ...
        new_user = User(name=name, email=email.lower(), password=hashed_password)
        db.session.add(new_user)
        db.session.commit()
        try:
            msg = Message(subject='Welcome to AgriBot 🌱', recipients=[email], body=f'Hello {name},\n\nWelcome to AgriBot! 🎉\n\nYou can now log in and start identifying crop diseases, getting prevention tips, and weather insights.\n\nHappy Farming!\n\n- AgriBot Team')
            mail.send(msg)
